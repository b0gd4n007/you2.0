...
export default function TaskScreen({ navigation }) {
  ...
  function onToggleRepeat() {
    ...
    setThreads((prev) => {
      ...
      const node = getNodeRef(updated, selectedLevel, selectedPath || []);
      if (!node) return prev;
      if (node.repeat) {
        delete node.repeat;
      } else {
        const base = node.targetDate ? new Date(node.targetDate) : new Date();
        node.repeat = {
          weekday: base.getDay(),
          hour: base.getHours(),
  ...
  return (
    ...
    >
      ...
      <ContextMenu
        ...
        hasRepeat={(selectedLevel && selectedPath && getNodeRef(threads, selectedLevel, selectedPath)?.repeat) ? true : false}
  ...
  />
      ...
      {chatVisible && (
        ...
        >
          <Pressable style={{ flex: 1 }} onPress={() => setChatVisible(false)} />
          <View
            style={{
              height: '65%',
